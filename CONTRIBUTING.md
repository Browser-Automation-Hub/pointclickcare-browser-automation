# Contributing to PointClickCare Browser Automation
...
- **Broken selectors**: PointClickCare updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real PointClickCare instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.POINTCLICKCARE_URL}/path/to/section`);
...
When you find working selectors for PointClickCare, document them:

```javascript
// PointClickCare selectors (verified YYYY-MM-DD)
...
PointClickCare updates its UI periodically. When selectors break:
