/**
 * anchor-cloud.js — AnchorBrowser cloud example for PointClickCare
 *
...
const { login_pcc, admit_resident } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting PointClickCare automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_pcc(page, {});
      log('Action 1 result:', data1);

      const data2 = await admit_resident(page, {});
